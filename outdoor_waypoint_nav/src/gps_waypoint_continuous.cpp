...
// initialize variables
	...
	int count = 0, waypointCount = 0, wait_count = 0, controller_num;
...
void odometry_CB(const nav_msgs::Odometry odom_msg)
{
...
int main(int argc, char** argv)
{
	...
	MoveBaseClient ac1("move_base1", true); 
	...
	{		 tol
		//Setting goal:
		  ...
		  if(iter < (waypointVect.end()-1))tol
		  {
			  iter++;
			  latiNext = iter->first;
		...
		//Build next goal if not on last waypoint
		  move_base_msgs::MoveBaseGoal goal_next = buildGoal(map_point, map_next, final_point); //initiate a move_base_msg called goal_next
